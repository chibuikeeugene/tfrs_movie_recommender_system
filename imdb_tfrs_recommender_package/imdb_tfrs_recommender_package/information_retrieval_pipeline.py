...
from .config.core import RETRIEVAL_MODEL
from  .processing import model_manager
import tensorflow as tf
import tensorflow_recommenders as tfrs
from . import model_data_pipeline
...
def run_model_training():
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(filepath=f'{RETRIEVAL_MODEL}/weights/trained_n_personalized_model',
    ...
    with tf.device('/GPU:0'): # setting tensorflow to run the fit operation on GPU
        model.fit(cached_train, epochs= 2, validation_data=cached_val, callbacks= callbacks)
