...
from ..config.core import RETRIEVAL_MODEL, RANKING_MDOEL, HYBRID_MODEL
...
def load_model(*, model:tf.keras.Model):
    """this function can be used to load any model
    
    args:
    * name:str - name of the model type(e.g., retrieval, ranking and hybrid) to be loaded
    * model:tf.keras.Model -  model to be loaded
    
    returns:
    a keras model
    
    """
    loaded_model = tf.saved_model.load(RETRIEVAL_MODEL)
    
    return loaded_model


#========= REMOVE OLD MODEL FUNCTION ====================== #
def remove_old_model(name:str ,model:tf.keras.Model):
    """use this method to remove a old models"""
    pass