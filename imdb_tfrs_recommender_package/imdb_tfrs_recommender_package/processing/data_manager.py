...
from ..config.core import DATASET, TRAINED_MODEL_DIR
...
# path to save csv_file
csv_path = f'{DATASET}/movies_data.csv'
...
def load_and_preprocess_dataset(*, filename1:str, filename2:str) -> tf.data.Dataset:
    ...
    * a tensor data
    ...
    users_data.rename(columns={'review date': 'review_date'}, inplace=True)

    data = users_data.merge(ratings_data, on='movieID')

    # save the new dataframe to a csv file - to be used in generating predictions later
    ...
    df['review date in unix'] = [datetime.datetime.timestamp(time) for time in df['review_date']]
    ...
    logger.info('wrapping things up with the final dataframe by removing review date and converting it to a tensor datatset...')
...
def load_and_preprocess_dataset_prod(*, data:Dict[str, Any]) -> tf.data.Dataset:
    ...
    # convert the review date to datetime
    user_df['review_date'] = pd.to_datetime(user_df['review_date'])

    # convert the datetime to just unix timestamp
    user_df['review date in unix'] = [datetime.datetime.timestamp(time) for time in user_df['review_date']]

    user_df = user_df.drop('review_date', axis=1)
