...
class FaissRetrievalIndex():
    ...
    * model: This is the already compiled and trained_model
    ...
    def search_top_k(self, user, k):
        """ perform similarity search of the query with the existing embeddings
        
        args:
        * user: a dict containing an id and a review date key which represents the query we intend to find a result for

        * k: the number of possible result to be returned
        """

        # get the user embeddings and convert it to numpy array
        user_embeddings = self.query_model(user)
...
def make_prediction(*, 
                    ...
                    movies_table:pd.DataFrame) -> list[str]:
    """
    this method generates a list of recommended movies

    args:
    * data: json data retrieved from an API response
    * embed_dim: int - the vector embeddings dimension
    * model - tf.keras.saved_Model
    * movies_table - dataframe or table that contains all the movie features
    
    return:
    * data: list[str] - a list of movie titles
    ...
    movie_att_tensor_map =  user_df_tensor.map(lambda x:
        {
        'movieID': x['movieID'],
    ...
    for movie in movie_att_tensor_map.batch(1).as_numpy_iterator():
    ...
    user_map = user_df_tensor.map(lambda x:
        {
        'userID': x['userID'],
        'review date in unix': x['review date in unix'],
        }
    )
    for user in user_map.as_numpy_iterator():
        ...
        recommended_movies = [movies_table[movies_table['movieID']== id]['originalTitle'].values[0] for id in recommended_movie_str]

    # Print the recommended movies for the user
    logger.info(f"Recommended Movies: {recommended_movies}")
    return {
        'version': 'v1',
